    ...
    >
      <div class="max-w-md w-full space-y-8 text-center">
        <div>
          ...
          <h2 class="mt-6 text-3xl font-extrabold text-gray-900">E-Mail Verifizierung</h2>
        </div>

        @if (loading()) {
          <div class="flex flex-col items-center">
            <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-teal-600"></div>
            <p class="mt-4 text-gray-600">E-Mail wird verifiziert...</p>
          </div>
        } @else if (success()) {
          <div class="rounded-md bg-green-50 p-6 border border-green-200">
            <div class="flex flex-col items-center">
              ...
              <h3 class="text-lg font-medium text-green-800">E-Mail verifiziert!</h3>
              <p class="mt-2 text-sm text-green-700">
                Ihre E-Mail-Adresse wurde erfolgreich bestätigt. Sie können sich nun in Ihr Konto
                einloggen.
              </p>
              <div class="mt-6">
                <a
          ...
          <div class="rounded-md bg-red-50 p-6 border border-red-200">
            <div class="flex flex-col items-center">
              ...
              <h3 class="text-lg font-medium text-red-800">Verifizierung fehlgeschlagen</h3>
...
export class VerifyEmailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!token) {
      ...
      this.error.set('Kein Verifizierungs-Token gefunden.');
      return;
    }
    ...
    this.authService.verifyEmail(token).subscribe({
      ...
      error: (err) => {
        this.loading.set(false);
        this.error.set(err.error?.detail || 'E-Mail-Verifizierung fehlgeschlagen.');
      },
    });
  }
}
